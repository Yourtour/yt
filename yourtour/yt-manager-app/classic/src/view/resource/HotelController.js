 ...
 */
Ext.define('yt_manager_app.view.resource.HotelController', {
    extend: 'yt_manager_app.view.widget.GeneralCRUDController',
    alias: 'controller.hotel',

    config: {
        popupWindowName: 'hotelPopupWindow',
    ...
    init: function () {
        var me = this,
            grid = me.lookupReference('hotel_crud_grid_paging');
    ...
    createNewWindow: function () {
        return new yt_manager_app.view.resource.HotelWindow();
    },

    dowithRecord: function (data) {
        if (data == null) {
            return new yt_manager_app.model.resource.Hotel({id: null});
        } else {
            return data;
        }
    },
    ...
    onTabChange: function (tabs, newTab, oldTab) {
        var me = this;
        var id = newTab.id;
        if (id == 'hotel-crud-tab') {
            // all member tab
            me.setCurrentGrid(me.lookupReference('hotel_crud_grid_paging'));
        } else if (id == 'hotel-cypher-tab') {
            // cyphere tab
            me.setCurrentGrid(me.lookupReference('hotel_cypher_grid_paging'));
        } else {
            // graph tab
            // TODO 图关系
        }
