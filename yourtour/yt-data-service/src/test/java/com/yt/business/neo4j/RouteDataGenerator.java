...
public class RouteDataGenerator extends DataGenerator{
	...
	public void createRelationWithPlace(){
		try {
			...
			long[] ids = new long[]{329l,336l,342l,1069l,15l};
			Map<String, Object> props = new HashMap<>();
			props.put("LEVEL", 0);
			for(int index = 0; index < ids.length; index++) {
				RouteMainBean route = new RouteMainBean();
				route.setGraphId(ids[index]);

				repository.createRelation(place, route, Constants.RELATION_TYPE_RECOMMEND, Direction.OUTGOING, props);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
			fail(ex.getMessage());
