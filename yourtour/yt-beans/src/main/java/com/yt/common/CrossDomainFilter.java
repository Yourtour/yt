...
public class CrossDomainFilter implements Filter {
	private static Log logger = LogFactory.getLog(CrossDomainFilter.class);
    ...
    public void init(FilterConfig config) throws ServletException{
        ...
        try {
        	...
        	logger.error("Cross-origin filter, could not parse '{}' parameter as integer: {}", x);
        }
        ...
        logger.info("Cross-origin filter configuration: " +
        ...
        if (origin != null && isEnabled(request)){
            if (originMatches(origin)){
                if (isSimpleRequest(request)){
                    logger.debug("Cross-origin request to {} is a simple cross-origin request, url=" +  request.getRequestURI());
                    handleSimpleResponse(request, response, origin);
                }else{
                    logger.debug("Cross-origin request to {} is a preflight cross-origin request, url=" + request.getRequestURI());
