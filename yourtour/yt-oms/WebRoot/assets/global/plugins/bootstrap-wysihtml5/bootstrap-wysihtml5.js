    ...
    Wysihtml5.prototype = {
        ...
        initInsertImage: function(toolbar) {
            toolbar.find('a[data-wysihtml5-command=insertImage]').click(function() {
                var fileField = $(this).siblings("input[type='file']");
                fileField.one("change", function(){
                    var formdata = new FormData(), context=$("#context").val();
                    var image = $(this)[0].files[0];
                    formdata.append("imageUrl", image);

                    $.Request.postFormData("/oms/common/image/activity/upload",formdata,function(result){
                        self.editor.currentView.element.focus();
                        self.editor.composer.commands.exec("insertImage", context + result.data);
                    })
                }).click();
            });
        },
