 ...
 * 
 */
public class RelationshipCrudGeneralOperate<S extends Neo4jBaseBean,T extends Neo4jBaseBean> implements CrudOperate<T>, RelationshipCrudOperate<S, T>{
	/** 静态变量：系统日志 */
	...
	public long count(Class<T> clazz) throws Exception {
		return template.count((Class<T>) clazz);
	}
	 ...
	 */
	@Override
	public List<T> get(Class<T> clazz, boolean loadRelations)
			throws Exception {
		@SuppressWarnings("unchecked")
	 ...
	 */
	@Override
	public List<T> getByPage(Class<T> clazz, int start, int limit,
	...
	public void delete(T bean) throws Exception {
		...
		T d = this.get((Class<T>)bean.getClass(), graphId);
		if (d != null) {
			// 先删除存在的关系
	...
	public void save(T neo4jBean, boolean saveRelation) throws Exception {
		...
		if (neo4jBean.getId() != null) {
			// 以ID是否存在为最优先
			bean = get(clazz, neo4jBean.getId());
		}
	...
	public void saveRelationsOnly(T neo4jBean,String[] relationshipFieldNames) throws Exception {
		...
		// 取出现有的关系
		T ori = this.get((Class<T>) neo4jBean.getClass(), neo4jBean.getId());
