Ext.define('YourTour.controller.StartupMainCtrl', {
     ...
     * 上传位置信息
     */
    submitPositionInfo: function () {
        if (this.getApplication().isWindowOS()) return;

        var me = this,
            localStore = this.getApplication().getLocalStorage();

        me.positionTask = Ext.create('Ext.util.DelayedTask', function () {
            if (localStore.get('user.profile') || localStore.get("setting.switch.position")) { //用户已登录，并且打开了位置分享开关
        ...
        me.positionTask.delay(5000); //每5秒上传一次位置信息
    },

    /** 一下部分代码包括了1）APP功能导航。2）APP主页入口代码 **/
