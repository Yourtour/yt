...
public class ExpertServiceImpl extends ServiceBase implements IExpertService {
	...
	@Override
	public List<RouteMainBean> getRecommendRoutes(Long uid, Long nextCursor,
			int limit) throws Exception {
		List<RouteMainBean> routes = new ArrayList<>();

		List<RouteTuple> tuples = routeRepository.getRecommendRoutes(uid,
				nextCursor, limit);
		if (CollectionUtils.isNotEmpty(tuples)) {
	...
	@Override
	public List<RouteMainBean> getServicedRoutes(Long expertId,
			Long nextCursor, int limit) throws Exception {
		List<RouteMainBean> routes = new ArrayList<>();

		List<RouteMainBeanRepository.OwnerRouteTuple> tuples = this.routeRepository
				.getRoutes(expertId, nextCursor, limit, "EXPERT");
		if (CollectionUtils.isNotEmpty(tuples)) {
